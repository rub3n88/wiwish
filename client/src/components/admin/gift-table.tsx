...
export function GiftTable({ gifts, onEdit }: GiftTableProps) {
  ...
  const filteredGifts = useMemo(() => {
    ...
    if (activeTab === "available") return gifts.filter(gift => gift.reservedBy === null);
    if (activeTab === "reserved") return gifts.filter(gift => gift.reservedBy !== null);
    return gifts;
  }, [gifts, activeTab]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/registry/${giftToDelete.registryId}/gifts`] });
  ...
  const columns: ColumnDef<Gift>[] = [
    ...
    },
    {
      ...
      header: "Precio",
      cell: ({ row }) => {
        return <span className="text-soft-gray-800">{row.original.price.toFixed(2)} €</span>;
      }
    },
    {
      id: "status",
      ...
      cell: ({ row }) => {
        ...
        return <GiftStatusBadge status={isReserved ? "reserved" : "available"} />;
      }
    },
    {
      id: "reservedBy",
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex justify-end space-x-2">
            <Button variant="ghost" size="icon" onClick={() => window.open(gift.url, "_blank")}>
            ...
            <Button variant="ghost" size="icon" onClick={() => handleDeleteClick(gift)}>
  ...
  const availableCount = gifts.filter(gift => gift.reservedBy === null).length;
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción eliminará permanentemente el regalo "{giftToDelete?.name}" y no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
