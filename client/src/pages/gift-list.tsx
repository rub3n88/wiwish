...
export default function GiftList() {
  ...
  const { data: gifts, isLoading: giftsLoading, error: giftsError } = useQuery({
    queryKey: [`/api/registry/${id}/gifts`],
  });
  
  // Filter gifts based on search query and category
  const filteredGifts = gifts ? gifts.filter((gift: Gift) => {
    ...
    const matchesCategory = filter.category === "all" || gift.category === filter.category;
  ...
  const availableCount = gifts ? gifts.filter((gift: Gift) => !gift.reservedBy).length : 0;
  const reservedCount = gifts ? gifts.filter((gift: Gift) => gift.reservedBy).length : 0;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center p-8 bg-white rounded-lg shadow-md">
          <h1 className="text-2xl font-bold text-soft-gray-800 mb-4">Lista no encontrada</h1>
          <p className="text-soft-gray-600 mb-6">Lo sentimos, la lista de regalos que buscas no existe o no está disponible.</p>
      ...
      <main className="flex-grow container mx-auto px-4 py-6">
        ...
        <div className="mb-6 bg-white rounded-lg p-4 shadow-sm flex justify-between items-center">
          <p className="text-soft-gray-700">
            <span className="font-bold text-baby-blue-600">{availableCount}</span> regalos disponibles | 
            <span className="font-bold text-baby-pink-600">{reservedCount}</span> regalos reservados
          </p>
          <div>
            <Button 
              variant="ghost" 
        ...
        {filteredGifts.length > 0 ? (
          <div className={`${isGridView ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4' : 'space-y-4'} mb-8`}>
          ...
          <div className="text-center py-12 bg-white rounded-lg shadow-sm mb-8">
            <p className="text-soft-gray-600">No se encontraron regalos que coincidan con tu búsqueda.</p>
          </div>
        )}
